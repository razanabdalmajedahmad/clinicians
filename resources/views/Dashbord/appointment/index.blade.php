...
@section('content')
    <div class="pagetitle">
        ...
        <div class="d-flex justify-content-between">
            ...
            <div>
                <a class="btn btn-primary" href="{{ route('appointment_createnew') }}"> Create new Appointment</a>
            </div>
        </div>
    </div>
                ...
                <div class="card p-4">
                    <div class="card-header">
                        @if (Session::has('success'))
                            <div class="alert alert-success mt-1" id="alert-session">
                                {{ Session::get('success') }}
                            </div>
                        @endif
                        @if (Session::has('error'))
                            <div class="alert alert-danger mt-1" id="alert-session-error">
                                {{ Session::get('error') }}
                            </div>
                        @endif
                        <div hidden id="alert-error">
    ...
    <script type="text/javascript">
        $(function() {
            ...
            var table = $('#datatable').DataTable({
                ...
                ajax: {
                    type: 'post',
                    url: "{{ route('appointment_list_post') }}",
                    beforeSend: function() {
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data.status) {
                                    table.draw();
                                    $('#alert-session').attr('hidden', true)
                                    $('#alert-session-error').attr('hidden', true)
